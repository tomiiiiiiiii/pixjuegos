...
Global
	premios_armas[30]=1,2,3,4,5,6,8,10,11,13,15,17,18,19,21,22,23,24,25,26,27,28,34;
...
Begin
	...
	file=fpg_general;
	graph=1;
	from alpha=0 to 255 step 15; frame; end
	while(accion!=-1)
		frame;
	end
...
Begin
	...
	let_me_alone();
	controlador(0);
	from i=1 to 4; stop_scroll(i); end
	reset();
	delete_text(all_text);
	loop
		if(!exists(type titulo)) j=titulo(); end
		put_screen(fpg_general,11);
		while(p[0].botones[b_2]) frame; end
		switch(menu("Jugar#Ayuda#Créditos#Salir",0))
			case 0: //jugar
				i=menu("2 jugadores#3 jugadores#4 jugadores",1);
				if(i!=-1)
					from j=1 to i+2; p[j].juega=1; end
					i=menu("1 victora#3 victorias#5 victorias",1);
					if(i!=-1)
						rondas=i+1;
						break;
					end
...
Process vista_jugador(jugador);
Begin
	graph=scroll_jugador[jugador];
	size=66;
	switch(jugador)
		case 1: x=320; y=180; end
		case 2: x=960; y=180; end
		case 3: x=320; y=540; end
		case 4: x=960; y=540; end
	end
	//z=-9999;
	z=100;
	loop
...
Process camara_personaje(jugador);
Private
	cam_min_x;
	cam_min_y;
	cam_max_x;
	...
	dest_y;
Begin
	cam_min_x=((ancho_pantalla/4)*3/2)-82;
	cam_min_y=((alto_pantalla/4)*3/2)-82;
	cam_max_x=(size_mapa*ancho_pantalla)-cam_min_x;
	cam_max_y=(size_mapa*alto_pantalla)-cam_min_y;
	...
	loop
		...
		scroll[jugador].x0=x-480;
		scroll[jugador].y0=y-270;
...
Begin
	...
	let_me_alone();
	//delete_text(all_text);
	from i=0 to 5; stop_scroll(i); end
	frame(3000);
	fade(100,100,100,8);
	...
	from x=1 to size_mapa;
		from y=1 to size_mapa;
			from i=1 to 4;
				zonas[i][x][y].tipo=0;
			end
		end
	...
	from i=1 to 4;
		zonas[i][centro][centro].tipo=1;
		zonas[i][1][1].tipo=1;
		zonas[i][size_mapa][1].tipo=1;
		zonas[i][1][size_mapa].tipo=1;
		zonas[i][size_mapa][size_mapa].tipo=1;
	end
	...
	from x=1 to size_mapa;
		from y=1 to size_mapa;
			from i=1 to 4;
				if(zonas[i][x][y].tipo==0)
					zonas[i][x][y].tipo=rand(2,num_zonas);
				end
				adorno_en_scroll((x*1280)-640,(y*720)-360,fpg_mapa,100+zonas[i][x][y].tipo,0,i);
			end
		end
	end
	...
	from i=1 to size_mapa;
		from j=1 to size_mapa;
			if(zonas[1][i][j].tipo!=1)
				from x=1 to 5;
					enemigos[k].id=enemigo((1280*(i-1))+640,(720*(j-1))+360,zonas[1][i][j].tipo-1,k++);
				end
				if(rand(0,1)==1)
					premio((i*1280)-640,(j*720)-360,1,premios_armas[rand(1,22)]);
					//premio((i*1280)-640,(j*720)-360,1,17);
				else
					premio((i*1280)-640,(j*720)-360,2,rand(1,11));
					//premio((i*1280)-640,(j*720)-360,2,contrato);
				end
			end
		end
	end
	...
	//num nivel
	adorno_en_scroll((centro*1280)-640,(centro*720)-360,0,write_in_map(fnt_texto,"B"+nivel,4),0,0); //arriba izquierda
	
	//jefe
	//enemigo(rand(40,(size_mapa*1280)-40),rand(40,(size_mapa*720)-40),100+rand(1,19),k++);
	
	//4 esquinas
	adorno_en_scroll(-41,-41,fpg_mapa,4,0,0); //arriba izquierda
	adorno_en_scroll(borde_nivel_x+71,-41,fpg_mapa,4,-90000,0); //arriba derecha
	adorno_en_scroll(-41,borde_nivel_y+71,fpg_mapa,4,90000,0); //abajo izquierda
	adorno_en_scroll(borde_nivel_x+71,borde_nivel_y+71,fpg_mapa,4,180000,0); //abajo derecha
	
	//bordes
	from i=0 to size_mapa-1;
		adorno_en_scroll(-41,(alto_pantalla/2)+(i*alto_pantalla),fpg_mapa,3,0,0);
		adorno_en_scroll(borde_nivel_x+71,(alto_pantalla/2)+(i*alto_pantalla),fpg_mapa,3,180000,0);
		
		adorno_en_scroll((ancho_pantalla/2)+(i*ancho_pantalla),-41,fpg_mapa,2,0,0);
		adorno_en_scroll((ancho_pantalla/2)+(i*ancho_pantalla),borde_nivel_y+71,fpg_mapa,2,180000,0);
	end
	...
	define_region(2,640,0,640,360);
	define_region(3,0,360,640,360);
	define_region(4,640,360,640,360);*/
	...
	from i=1 to 8;
		if(p[i].juega)
			...
			//start_scroll(i,fpg_mapa,1,0,i,15);
			//scroll[i].camera=camara_personaje(i);
		end
	end
	...
	loop
		...
		if(i=ha_ganado_alguien())
			...
			jugador=0;
			if(jugador==5)
				mensaje("Error culpa de Carles, seguro.");
			else
				mensaje("El jugador "+i+" ha ganado esta ronda");
			end
			...
			if(p[i].victorias<rondas)
				nivel++;
				mazmorra();
			else
				let_me_alone();
				delete_text(all_text);
				put_screen(fpg_general,20+i);
				from i=1 to 300; frame; end
				pon_menu();
			end
		end
		frame;
...
Begin
	...
	p[jugador].id=id;
	switch(jugador)
		case 1: x=640; y=360; end
		case 2: x=(size_mapa*1280)-640; y=360; end
		case 3: x=640; y=(size_mapa*720)-360; end
		case 4: x=(size_mapa*1280)-640; y=(size_mapa*720)-360; end
	end
	...
	loop
		...
		if((p[jugador].botones[4] and accion!=HERIDO and (pulsando==0 or p[jugador].armas[1]==METRALLETA or p[jugador].armas[1]==LANZALLAMAS)) and retraso==10)
			retraso=0;
			pulsando=1;
			arma(p[jugador].armas[1]);
		end
		if((p[jugador].botones[5] and accion!=HERIDO and (pulsando==0 or p[jugador].armas[2]==METRALLETA or p[jugador].armas[2]==LANZALLAMAS)) and retraso==10)
			retraso=0;
			pulsando=1;
...
Begin
	...
	//grafico iconos lateral derecho
	x=590;
	y=180;
	if(jugador==2 or jugador==4) x+=640; end
	if(jugador==3 or jugador==4) y+=360; end
	grafico_en_pantalla(0,6,x,y);
	
	//grafico iconos lateral izquierdo
	if(jugador==1 or jugador==3) x=40; else x=640+40; end
	if(jugador==1 or jugador==2) y=140; else y=360+140; end
	grafico_en_pantalla(0,8,x,y);
	...
	if(jugador==2 or jugador==4) x+=640; end
	if(jugador==3 or jugador==4) y+=360; end
	if(p[jugador].victorias>0)
		from i=1 to p[jugador].victorias;
			grafico_en_pantalla(fpg_objetos,100,x+(i*30),y);
		end
	end
	
	//numero
	if(jugador==1 or jugador==3) x=320; else x=640+320; end
	if(jugador==1 or jugador==2) y=50; else y=360+50; end	
	write_int(fnt_puntos,x,y,4,&p[jugador].distancia_enemigo);
	p[jugador].distancia_enemigo=100;
	while(exists(p[jugador].id))
		p[jugador].distancia_enemigo=2000;
		x=p[jugador].id.x;
		y=p[jugador].id.y;
		from i=1 to 4;
			if(exists(p[i].id) and i!=jugador)
				j=get_dist(p[i].id)/200;
				if(j<p[jugador].distancia_enemigo)
					p[jugador].distancia_enemigo=j;
				end
			end
		end
		if(p[jugador].distancia_enemigo==2000)
			p[jugador].distancia_enemigo=0;
		end
		frame(300);
	end
End

Process grafico_en_pantalla(file,graph,x,y);
Begin
	z=-9;
	loop
		frame;
...
Begin
	...
	write_int(fnt_stats,x+offset_x+20,y,0,&p[jugador].nivel);	
	y+=20;
	write(fnt_stats,x,y,0,"XP");
	id_texto=write(fnt_stats,x+offset_x+20,y_mander,0,stringmander);
	y_mander=y;
	y+=35;
	write_int(fnt_stats,x+offset_x,y,0,&p[jugador].ataque);
	y+=offset_y;
	write_int(fnt_stats,x+offset_x,y,0,&p[jugador].defensa);
	y+=offset_y;
	write_int(fnt_stats,x+offset_x,y,0,&p[jugador].velocidad);
	y+=offset_y;
	write_int(fnt_stats,x+offset_x,y,0,&p[jugador].suerte);
	loop
		if(i!=p[jugador].experiencia or j!=p[jugador].xp_siguiente)
			i=p[jugador].experiencia;
			j=p[jugador].xp_siguiente;
			stringmander=""+p[jugador].experiencia+"/"+p[jugador].xp_siguiente;
...
Begin
	...
	if(jugador==3 or jugador==4) y+=360; end
	graph=7;
	write_int(fnt_stats,x,y,4,&p[jugador].vida);
	loop
		if(p[jugador].vida>0)
			size_x=(100/100*p[jugador].vida)/2;
		else
			size_x=0;
		end
		frame;
...
Begin
	...
	y=-20;
	switch(num)
		case 1: file=fpg_armas2; end
		case 2: file=fpg_armas2; end
		case 3: file=fpg_objetos; end
	end
...
Begin
	...
	switch(tipo)
		...
		case BOLA_RAYOS: sonido(16); ataque=3; tipo_ataque=RAYO; velocidad=2; angle=rand(0,360)*1000; end
		case AGUJAS_HIELO: sonido(19); ataque=4; tipo_ataque=HIELO; velocidad=1; angle=rand(0,360)*1000; end
		case MINA_APAGADA: ataque=0; tipo_ataque=IMPACTO; velocidad=1; end
	end
	...
	if(tipo==AGUJAS_HIELO)
		alpha=255;
		size=100;
		while(anim<40 and exists(father))
			while_not_ready();
			alpha-=2;
			size+=6;
			x=father.x;
	...
	//MINA
	if(tipo==MINA_APAGADA)
		x=father.x;
		y=father.y;
		z=5;
		while(exists(father))
			while_not_ready();
			if(anim>30) graph=33; else graph=34; end
			if(anim<60) anim++; else anim=0; end
			if(i<60)
				i++;
			elseif(i==60)
				father.accion=QUIETO;
			...
			if(j=collision(type personaje_piernas))
				if(jugador!=j.jugador) break; end
			end
			if(j=collision(type enemigo))
				break;
			end
			frame;
		end
	...
	if(tipo==SHURIKEN or tipo==FLECHA or tipo==PIEDRA
	 ...
	 tipo==BOLA_RAYOS or tipo==BALAS)
		...
		loop
			...
			if(tipo==BOOMERANG or tipo==SHURIKEN)
				angle+=20000; 
			else
				switch(direccion)
					case 0: angle=0; end
			...
			if(accion==-1)
				...
				if(tipo==SHURIKEN or tipo==FLECHA or tipo==PIEDRA or tipo==BALAS or tipo==LLAMAS or tipo==MISIL_BAZOOKA
					...
					if(cambiada_accion==0 AND father.accion==ATACANDO)
						father.accion=QUIETO; 
						cambiada_accion=1;
					end
					return;
			...
			if(father.accion==QUIETO) cambiada_accion=1; end
			if(anim>300)
	...
	if(exists(father) and cambiada_accion==0)
		if(father.accion==ATACANDO) father.accion=QUIETO; end
	end
End
...
Begin
	...
	loop
		...
		if(alpha<160) alpha+=20; end
		if(alpha=>160)
			id_enemigo=personaje_enemigo_mas_cercano();
			if(exists(id_enemigo))
				if(p[id_enemigo.jugador].vida<=0)
					id_enemigo=0;
				else
					persigue_personaje_enemigo(id_enemigo);
				end
		...
		if(retraso<140)
			...
			if(exists(id_enemigo))
				if(collision(id_enemigo))
					...
					p[id_enemigo.jugador].vida-=p[jugador].ataque;
...
Function personaje_enemigo_mas_cercano();
Private
...
Begin
	...
	if(!exists(type personaje))
		...
		from i=1 to 8;
			if(p[i].juega and p[i].vida>0 and exists(p[i].id) and i!=jugador)
	...
	loop
		...
		if(accion==1)
			if(tipo==1)
				...
				i=p[jugador].objeto;
				if(num==CONTRATO) p[jugador].objeto=0; frame;end 
...
Begin
	from i=1 to 4;
		if(p[i].juega)
			if(p[i].id)
				if(get_dist(p[i].id)>ancho_pantalla*1.8)
					return 1;
				else 
					return 0;
				end
...
Process adorno_en_scroll(x,y,file,graph,angle,mapa);
Private
	graph_orig;
Begin
	z=500;
	...
	graph_orig=graph;
	switch(mapa)
		case 0: cnumber=c_1+c_2+c_3+c_4; end
		case 1: cnumber=c_1; end
		case 2: cnumber=c_2; end
		case 3: cnumber=c_3; end
		case 4: cnumber=c_4; end
	end
	loop
		frame(10000);
	end
...
Begin
	...
	if(file==0 and graph>999) unload_map(0,graph); end
End
...
Begin
	jugador=father.jugador;
	if(jugador<1 or jugador>4) return 0; end
	zona_x=(x/1280)+1;
	zona_y=(y/720)+1;
	if(map_get_pixel(fpg_mapa,200+zonas[jugador][zona_x][zona_y].tipo,x%1280,y%720)!=map_get_pixel(fpg_mapa,200+zonas[jugador][zona_x][zona_y].tipo,1,1))
		return 1;
	end
	return 0;
End
...
//					knockback=knockback/2;
	...
	p[ultimo_atacante].experiencia+=enemigos[num_enemigo].experiencia;
End
...
Function persigue_personaje_enemigo(id_enemigo);
Begin
	if(exists(id_enemigo))
		if(p[id_enemigo.jugador].vida>0)
			if(id_enemigo.x<father.x)
...
Begin
	...
	from i=1 to 4;
		if(p[i].juega)
			if(exists(p[i].id))
				if(get_dist(p[i].id)<ancho_pantalla)
					return 1;
				else
					return 0;
				end
...
Begin
	from i=1 to 4;
		...
		p[i].armas[2]=0;
		p[i].objeto=0;
		p[i].distancia_enemigo=0;
		p[i].victorias=0;
	end
	nivel=1;
End

Function ha_ganado_alguien();
Begin
	if(ready)
		from i=1 to 4;
			//if(!exists(p[i].id)) j++; end
			if(p[i].vida<1) j++; end
		end
		if(j==jugadores-1)
			from i=1 to 4;
				if(p[i].vida>0) return i; end
			end
		elseif(j==jugadores)
			return 5;
		else
