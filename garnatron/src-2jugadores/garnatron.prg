...
begin
	...
	if(cosa==2) //creditos
		...
		timer[2]=0;
		while(timer[2]<600)
			if(p[1].id.x<320) p[1].id.x+=1; end
			scroll.x0+=3;
			frame;
		end
		...
		timer[2]=0;
		while(timer[2]<600)
			if(p[1].id.x<850) p[1].id.x+=1; end
			scroll.x0+=3;
			frame;
		end
		...
		timer[2]=0;
		while(timer[2]<600)
			if(p[1].id.x<850) p[1].id.x+=2; end
			scroll.x0+=3;
			frame;
		end
			...
			switch(num_menu)
				case 0: //general
					switch(opcion_actual)
						case 1:
							p[1].vidas=3;
							if(jugadores==2) p[2].vidas=3; else p[2].vidas=-1; end
							juego(1);
						end
						case 2:
							p[1].vidas=save.vidas;
							juego(save.nivel);
						end
						case 3:
							menu(1);
...
if(nivel==1)
	...
	LOOP
		...
		if(pausa==0)
			...
			if(p[1].vidas<0 and p[2].vidas<0)
				delete_text(all_text);
				signal(p[1].id,s_kill);
				graph=20;
				direccion=1;
				size=200;
