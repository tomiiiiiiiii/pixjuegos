...
Global
	...
	string savegamedir;
	string developerpath="/.PiXJuegos/ruzafa/";
	string lang_suffix="";
...
Begin
	...
	if(os_id==1003)
		savegamedir="/data/data/com.pixjuegos.ruzafa/files/";
		mkdir(savegamedir);
	end
...
Function carga_fpgs();
Begin
	fpg_ruzafa[1]=load_fpg("fpg/ruzafa1.fpg");
	fpg_ruzafabici[1]=load_fpg("fpg/ruzafa1bici.fpg");
	fpg_pato[1]=load_fpg("fpg/pato1.fpg");
	fpg_patobici[1]=load_fpg("fpg/pato1bici.fpg");
	fpg_puntos[1]=load_fpg("fpg/puntos1.fpg");
	if(posibles_jugadores>1) //ahorro de recursos!
		from i=2 to 4;
			fpg_ruzafa[i]=load_fpg("fpg/ruzafa"+i+".fpg");
			fpg_ruzafabici[i]=load_fpg("fpg/ruzafabici"+i+".fpg");
			fpg_pato[i]=load_fpg("fpg/pato"+i+".fpg");
			fpg_patobici[i]=load_fpg("fpg/patobici"+i+".fpg");
			fpg_puntos[i]=load_fpg("fpg/puntos"+i+".fpg");
		end
	end

	from i=1 to 7;
		fpg_enemigo[i]=load_fpg("fpg/enemigo"+i+".fpg");
		recoloca_centros_personaje(fpg_enemigo[i]);
	end
	
	from i=1 to 4;
		recoloca_centros_personaje(fpg_ruzafa[i]);
		recoloca_centros_personaje(fpg_ruzafabici[i]);
		recoloca_centros_personaje(fpg_pato[i]);
		recoloca_centros_personaje(fpg_patobici[i]);
	end
...
Begin
	...
	if(fuego_amigo or ops.truco_ruzafa==1 or ops.truco_hamburguesas==1)
		con_puntos=0;
	else
		con_puntos=1;
	end
	...
	if(modo_juego==modo_supervivencia and ops.truco_hamburguesas<0)
		if(timer[0]>60*3*100) //si ha aguantado más de 3 minutos, le damos el cheto hamburguesa
...
Begin
	...
	if(ops.truco_ruzafa<0)
		ops.truco_ruzafa=0; //modo matajefes disponible
		guarda_opciones();
